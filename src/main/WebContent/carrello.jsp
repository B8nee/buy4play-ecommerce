                ...
                <% if (cart.isEmpty()) { %>
                    ...
                    <% } else { %>
                        <table border="0" cellpadding="10" style="width:100%; border-collapse: collapse;">
                            ...
                            <% for (CartItem item : cart.getItems()) { Prodotto p=item.getProdotto(); %>
                                <tr>
                                    <td>
                                        <%= p.getNome() %>
                                    </td>
                                    ...
                                    <td>
                                        <form action="carrello" method="get" style="display:inline;">
                                            ...
                                            <input type="number" name="qty" value="<%= item.getQuantita() %>" min="1"
                                                max="99" style="width:60px;">
                                            <input type="submit" value="Aggiorna" class="btn"
                                                style="padding:0.2rem 0.5rem;">
                                        </form>
                                    </td>
                                    <td>&euro; <%= String.format("%.2f", item.getSubtotale()) %>
                                    </td>
                                    <td><a href="carrello?action=remove&id=<%= p.getId() %>" class="btn"
                                            style="background:#aaa;">Rimuovi</a></td>
                                </tr>
                                <% } %>
                                    <tr style="font-weight: bold;">
                                        <td colspan="3" align="right">Totale:</td>
                                        <td>&euro; <%= String.format("%.2f", cart.getTotal()) %>
                                        </td>
                                        <td></td>
                                    </tr>
                        </table>
